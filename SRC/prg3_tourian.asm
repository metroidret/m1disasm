...
UpdateRoomSpriteInfo_Tourian_RinkaSpawner:
    ...
    lda RinkaSpawners.1.status,x
    bmi @RTS
    ; exit if rinka is in the current nametable
    lda RinkaSpawners.1.hi,x
    eor $02
    lsr
    bcs @RTS
...
@subroutine:
    ...
    lda RinkaSpawners.1.status,y
    bmi RTS_A15D
    
    ; exit if RinkaSpawners.1.hi == bit 0 of FrameCount
    ; (maybe to alternate which rinka spawner is processed each frame?)
    lda RinkaSpawners.1.hi,y
    eor FrameCount
    lsr
    bcc RTS_A15D
...
@slotFound:
    ...
    lda RinkaSpawners.1.hi,y
    sta EnHi,x
    lda RinkaSpawners.1.posIndex,y
    asl
    ora RinkaSpawners.1.status,y
    tay
    lda RinkaSpawnPosTbl,y
    jsr SpawnRinka_InitPositionXY
    ; increment rinka spawner position id
    ldx PageIndex
    inc RinkaSpawners.1.posIndex,x
    lda RinkaSpawners.1.posIndex,x
    cmp #$06
    bne RTS_A1DA
    lda #$00
LA1D8:
    sta RinkaSpawners.1.posIndex,x
RTS_A1DA:
    rts
