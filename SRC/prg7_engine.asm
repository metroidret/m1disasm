...
;Clear RAM at $0000-$07FF.
    ...
    lda #MMC1CTRL_MIRROR_VERTI | MMC1CTRL_PRGFIXED_C000 | MMC1CTRL_PRGBANK_16K | MMC1CTRL_CHRBANK_8K
    sta MMC1CTRL_ZP
    ...
    lda #SND_CHN_SQ1 | SND_CHN_SQ2 | SND_CHN_TRI | SND_CHN_NOISE
    sta SND_CHN
...
CheckVRAMStructBufferWrite:
    ...
    lda #<VRAMStructBuffer
    sta Temp00_VRAMStructPtr
    lda #>VRAMStructBuffer
    sta Temp00_VRAMStructPtr+1
    ;write it to PPU.
    jsr VRAMStructWrite
...
EndVRAMStruct: ; 07:C376
    ;If VRAM structure buffer is full or all bytes written, add #$00 as last byte to the PPU byte string.
    lda #$00
    sta VRAMStructBuffer,x
...
MoreInit:
    ...
    stx UpdatingWeaponProjectile    ;No weapon projectiles are being updated.
    ;A=0.
    txa
...
SamusInit:
    ...
    sty Samus.status                   ;Set Samus status as fading onto screen.
    ldx #$00
...
GameEngine:
    ...
    bne RTS_C92A
    ; samus is dead
    ;Exit if Samus is still exploding
    lda Samus.animDelay
    bne RTS_C92A
    ;Samus is done exploding.
    ;Turn off music.
    jsr SilenceMusic
...
UpdateWorld:
    ...
    jsr UpdateWeaponProjectiles     ;($D4BF)Display of bullets/missiles/bombs.
...
SamusJump_CheckFire:
    ...
    bne @notAimingUp
    jmp FireWeaponProjectileUpwards

@notAimingUp:
    jsr FireWeaponProjectileForwards
...
FireWeaponProjectile:
    lda Joy1Status
    and #BUTTON_UP
    beq FireWeaponProjectileForwards
    jmp FireWeaponProjectileUpwards


; search for open samus weapon projectile slot
; returns zero flag set if slot was found
; returns slot low byte in y
SearchOpenWeaponProjectileSlot:
    ; loop through all samus weapon projectile slots
    ldy #WeaponProjectiles.0 - Objects
    @loop:
        lda Objects.0.status,y
        beq @slotFound
        jsr Yplus16
        bne @loop
    ; all samus weapon projectile slots are occupied
    ; return clear zero flag
    iny
    rts

@slotFound:
    ; found open samus weapon projectile slot
    ; clear is hit
    ...
    beq @endIf_A
        ...
        cpy #WeaponProjectiles.0 - Objects
    @endIf_A:
    rts


FireWeaponProjectileForwards:
    ; exit if there is a metroid on samus
    lda MetroidOnSamus
    bne @exit

    ; search for open samus weapon projectile slot
    jsr SearchOpenWeaponProjectileSlot
    ...
    lda #$0C
    sta WeaponProjectileDieDelay,y
    ldx SamusDir
...
CheckHorizontalMissileLaunch:
    ...
    ; exit if not weapon projectile slot $03D0 (missile)
    cpy #WeaponProjectiles.0 - Objects
...
CheckMissileLaunchCommon:
    ...
    lda #$FF
    sta WeaponProjectileDieDelay,y     ; # of frames weapon projectile should last
    dec MissileCount
...
UpdateWeaponProjectiles:
    ldx #$D0
    jsr DoOneWeaponProjectile
    ldx #$E0
    jsr DoOneWeaponProjectile
    ldx #$F0
    ; fallthrough

DoOneWeaponProjectile:
    stx PageIndex
    lda Objects.0.status,x
    jsr JumpEngine
        .word ExitSub     ;($C45C) rts
...
CheckBulletStat:
    ...
    bcc @collided
        ...
        dec WeaponProjectileDieDelay,x     ; decrement bullet timer
        bne DrawBullet
...
UpdateBullet_ExplodeIfHitSprite:
    ; exit if weapon projectile didn't hit anything
...
GotoWeaponProjectileHitDoorOrStatue:
    jmp WeaponProjectileHitDoorOrStatue
...
UpdateBullet_CollisionWithBG:
    ...
    cmp #$4E
    beq GotoWeaponProjectileHitDoorOrStatue
    jsr CheckBlastTile
    bcc RTS_X081
    clc
    jmp IsBlastTile
...
BombInit:
    ...
    jsr InitObjAnimIndex
    lda #$18        ; fuse length :-)
    sta WeaponProjectileDieDelay,x
...
BombCountdown:
    ...
    lsr
    bcc Lx085
    dec WeaponProjectileDieDelay,x
...
BombExplode:
    inc WeaponProjectileDieDelay,x
...
BombExplosion_CollisionWithBG:
    ...
    sta $0B
    ; bomb center if WeaponProjectileDieDelay == 1
    ldx PageIndex
    ldy WeaponProjectileDieDelay,x
    dey
    beq Lx088
    dey
    bne Lx089
        ; WeaponProjectileDieDelay == 2, bomb 2 tiles up
        lda #$40
        jsr LD78B
        ; branch always
        txa
...
Lx089:
    dey
    bne Lx092
        ; WeaponProjectileDieDelay == 3, bomb 2 tiles down
        lda #$40
        jsr LD77F
        ; branch always
        txa
    ...
    dey
    bne Lx095
        ; WeaponProjectileDieDelay == 4, bomb 2 tiles left
        lda #$02
        jsr LD78B
        ; branch always
        txa
    ...
    dey
    bne Exit7
        ; WeaponProjectileDieDelay == 5, bomb 2 tiles right
        lda #$02
        jsr LD77F
        ; branch always
        txa
    ...
    bcc Lx097
        cmp #$A0
        bcs Lx097
        jsr IsBlastTile_SkipCheckUpdatingWeaponProjectile
    Lx097:
    pla
    tax
Exit7:
...
UpdateAllStatues:
    ...
    ldx #RidleyStatueStatus - KraidStatueStatus + (Statue - Objects)
    jsr UpdateStatue
...
UpdateStatue_StartRaising:
    ...
    beq @exit
        ; statue is hit by samus's weapon projectiles
...
DrawEnemy_NotBlank:
    ...
    bne Lx146
        ...
        ldx PageIndex
        inc Ens.0.chunkyExplosionTimer,x
        lda Ens.0.chunkyExplosionTimer,x
        pha
    ...
    jsr IsObjectVisible
    ; write this flag in bit 1 of Ens.0.data05
    txa
    asl
    sta Temp08_RadiusY
    ldx PageIndex
...
LDE60:
    ...
    lda Objects.0.hi,x                     ;
    sta Temp06_PositionHi           ;
    lda Objects.0.animFrame,x              ;Load A with index into ObjFramePtrTable.
...
;Special case for Samus exploding.
    ...
    ldx PageIndex                   ;X=0.
    lda #sa_Dead2                   ;
    sta Objects.0.status,x                 ;Move to next part of the death handler.
    lda #$28                        ;
    sta Objects.0.animDelay,x                 ;Set animation delay for 40 frames(.667 seconds).
...
LDEBC:
    ...
    lda (Temp00_FramePtr),y         ;
    sta Objects.0.radiusY,x                   ;Get vertical radius in pixels of object.
...
ExplodeYDisplace:
    ...
    bne LDF8F            ;Is Samus the object exploding? if so, branch.
        ...
        adc Ens.0.speedSubPixelY,y                 ;Increment every frame enemy is exploding. Initial=#$01.
...
VerticalRoomCentered: ; ($E21B)
    ...
    inx                             ;X=1.
    lda Samus.x                        ;Did Samus enter in the right hand door?-->
...
@dontStartFalling:
    ...
    bpl @else_B
        ...
        ;Subtract Samus.speedY to see if maximum speed has been exceeded.
        sbc Samus.speedY
        cmp #$06
...
UpdateNameTable: ; 07:E590
    ...
    jsr GetNameAddrs
    ora Temp00_RoomRAMPtr+1
    sta Temp02_PPURAMPtr+1
...
WritePPUAttribTbl:
    ...
    stx VRAMStructBufferIndex                 ;Store updated VRAM structure buffer index.
...
LE733:
    ...
    sta $00                         ;Store result.
    lda $01                         ;
    adc #>WorldMapRAM             ;Add #$7000 to result.
...
LE7E6:
    ...
    ; branch if bullet hit solid blank tile
    cmp #$4E
    beq WeaponProjectileHitDoorOrStatue
    jsr GotoUpdateBullet_CollisionWithMotherBrain
    jsr CheckBlastTile
...
WeaponProjectileHitDoorOrStatue:
    ; exit if we aren't updating a samus weapon projectile
    ldx UpdatingWeaponProjectile
    beq ClcExit
    ldx #$06
    ; go through all doors
    @loop:
        ; check if weapon projectile tile column is the same as door tile column
        ; otherwise check next door
        lda Temp04_RoomRAMPtr+1
        eor DoorRoomRAMPtr+1,x
        and #$04
        bne @next
        lda Temp04_RoomRAMPtr
        ...
        bcs @blueDoor
            ; missile door
            ldx PageIndex
            ; check if weapon projectile is a missile or missile explosion
            lda Objects.0.status,x
...
IsBlastTile:
    ldy UpdatingWeaponProjectile
    beq Exit18
IsBlastTile_SkipCheckUpdatingWeaponProjectile:
    tay
...
DrawObject:
    ...
    lda RoomRAMPtr+1                ;(start at beginning of the room).
    sta RoomRAMWorkPtr+1            ;
...
LEA80:
    ...
    inc RoomRAMWorkPtr+1            ;pointer, else branch to draw object.
...
LEB4D:
    ...
    sta Ens.0.y,x                ;Store enemy y position.
    ...
    sta Ens.0.x,x                ;Store enemy x position.
...
DeleteOffscreenRoomSprites:
    ...
    lsr
    tay
    ; non-beam weapon projectiles
    ldx #WeaponProjectiles.0 - Objects
    jsr WeaponProjectile_RemoveIfOffScreen
    ldx #WeaponProjectiles.1 - Objects
    jsr WeaponProjectile_RemoveIfOffScreen
    ldx #WeaponProjectiles.2 - Objects
    jsr WeaponProjectile_RemoveIfOffScreen
...
; y = current nametable
WeaponProjectile_RemoveIfOffScreen:
    ; exit if weapon projectile doesn't exist or is a beam
    lda Objects.0.status,x
    cmp #wa_BulletExplode+1
    bcc @RTS

    ; exit if weapon projectile is in current nametable
    tya
    eor Objects.0.hi,x
    ...
    ; weapon projectile exists but is not on screen
...
SpawnMellows:
    ...
    sta EnMellowHandlerExtra.resetAnimIndex
...
;Figure out cart RAM address of the byte containing the relevant bits.
    ...
    and #$07
    sta Temp02_AttribPtr+1
    lda Temp02_AttribPtr
    lsr
    lsr
    and #$38
    ora Temp02_AttribPtr+1
    ora #$C0
    sta Temp02_AttribPtr
    lda #$63
    sta Temp02_AttribPtr+1
...
; mellow <--> bullet/missile/bomb detection
    ...
    Lx261:
        ...
        beq Lx266
            ...
            Lx262:
            ; check for collision with samus's weapon projectiles
            ldy #WeaponProjectiles.0 - Objects
            Lx263:
                ; try next weapon projectile if this one is not active
                lda Objects.0.status,y
                beq Lx265
                ; try next weapon projectile if it is not a bullet, unknown7, bomb or missile
                ...
                bne Lx265
                Lx264:
                    ; weapon projectile is of the right type
                    ; hit mellow if they collided
                    jsr CollisionDetectionMellow_CheckWithObjectYSlot
                    jsr CollisionDetectionMellow_ReactToCollisionWithWeaponProjectile
                Lx265:
                jsr Yplus16
                bne Lx263
        Lx266:
...
Lx269:
    ...
    LF09F:
        ...
        ; skip weapon projectile collision if enemy is a pickup
        jsr GetEnemyXSlotPosition
        lda EnsExtra.0.status,x
        cmp #enemyStatus_Pickup
        beq Lx274

        ; first weapon projectile slot
        ldy #WeaponProjectiles.0 - Objects
        Lx271:
            ...
            Lx272:
                ...
                jsr CollisionDetectionEnemy_ReactToCollisionWithWeaponProjectile
            Lx273:
            ; next weapon projectile slot
            jsr Yplus16
            bne Lx271
    Lx274:
...
CollisionDetectionEnemy_ReactToCollisionWithWeaponProjectile:
    ; exit if collision didn't happen
    bcs Lx293

    ; save weapon projectile action to enemy
    lda Objects.0.status,y
    sta Ens.0.weaponAction,x
    ; set weapon projectile is hit flag
    jsr SetWeaponProjectileIsHit
Lx291:
...
CollisionDetectionMellow_ReactToCollisionWithWeaponProjectile:
    bcs ClearHealthChange@RTS
    jsr SetWeaponProjectileIsHit
...
UpdateEnemy_UpdateEnData05Bit6:
    ...
    eor FrameCount
    lsr
    ; shift carry into bit 6 of Ens.0.data05
    tya ;543210-7
    ror ;X543210-
    ror ;7X543210
    sta Ens.0.data05,x
...
UpdateEnemy_Active_BranchB: ; 07:F40A
    jsr EnemyReactToSamusWeaponProjectile
UpdateEnemy_Explode:
...
UpdateEnemy_Hurt:
    ...
    bne @exit
        ...
        ; Restore upper two bits of Ens.0.specialAttribs
        lda Ens.0.prevStatus,x
        tay
        and #$C0
...
RemoveEnemyIfItIsInLava:
    ...
    ldx PageIndex
    cmp #$02
    bcc EnemyReactToSamusWeaponProjectile@RTS
...
EnemyReactToSamusWeaponProjectile:
    ...
    lda Ens.0.isHit,x
    and #$20
    beq EnemyReactToSamusWeaponProjectile@RTS
...
ExplodeEnemy:
    ...
    sta Ens.0.chunkyExplosionTimer,x
...
@exitLoop:
    ...
    ldy PageIndex
    lda Ens.0.y,y
    sta EnExplosions.0.y - EnExplosions + Ens,x
    lda Ens.0.x,y
...
UpdateEnemy_Resting_UpdateEnData1F: ; 07:F676
    ; load L977B entry * 2
    jsr LoadTableAt977B
...
UpdateEnemy_ForceSpeedTowardsSamus:
    ...
    ; clear bit 0 of Ens.0.data05 (x axis flip)
    lda #~$01
    jsr AndEnData05
...
@skipXAxis:
    ...
    ; clear bit 2 of Ens.0.data05 (y axis flip)
    lda #~$04
    jsr AndEnData05
...
UpdateEnemy_EnData05DistanceToSamusThreshold:
    ...
    ; push to y
    tay
    ; unset bit 4 and bit 3 of Ens.0.data05 and exit if enemy is invisible
    lda Ens.0.data05,x
    and #$02
    beq Lx345
    ...
    ; pop from y
    tya
    ; mask out bit 3 from Ens.0.data05
    ldy #~$08
...
UpdateEnemy_Resting_TryBecomingActive:
    ...
    bne @RTS
    ; delay is zero
    ; branch if bit 3 of Ens.0.data05 is set
    lda Ens.0.data05,x
    and #$08
    bne @becomeActive
...
Lx362:
    ...
    ; if bit 0 of Ens.0.data0A is set, invert x speed
    tay
    lda Ens.0.data0A,x
    lsr
    php
...
UpdatePipeBugHole:
    ...
    inx
    beq UpdateAllEnemyExplosions@RTS
...
InitEnemyFacingDirectionAxisAndDelay: ; 07:FB7B
    ; rotate bit 7 of L977B into bit 7 of Ens.0.data05
...
UpdateAllMellows:
    ...
    lda EnMellowHandlerExtra.resetAnimIndex
